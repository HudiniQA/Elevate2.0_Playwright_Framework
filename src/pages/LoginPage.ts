...
export default class LoginPage {
    ...
    async verifyLoginWithIncorrectCredentials() {
        ...
        await expect(this.page.locator(this.incorrectCredentialsTxt)).toHaveText('Login failed - Incorrect username or password.', {
            timeout: 90000,
        }).catch((error) => {
    ...
    async verifyLoginWithInvalidCredentials() {
        await expect(this.page.locator(this.userdoesntexistTxt)).toHaveText('Login failed - User does not exist.', {
            timeout: 90000,
        }).catch((error) => {
